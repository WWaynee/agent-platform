...
func GetSessionByID(ctx context.Context, tenantID, id uint64) (*model.Session, error) {
	var s model.Session
	err := DB.WithContext(ctx).Where("id = ? AND tenant_id = ?", id, tenantID).First(&s).Error
	if err != nil {
		return nil, err
	}
	return &s, nil
...
func ListSessions(ctx context.Context, tenantID, userID uint64, page, pageSize int) ([]model.Session, int64, error) {
	...
	query := DB.WithContext(ctx).Model(&model.Session{}).Where("tenant_id = ?", tenantID)
	if userID > 0 {
...
func DeleteSession(ctx context.Context, tenantID, id uint64) error {
	return DB.WithContext(ctx).Where("id = ? AND tenant_id = ?", id, tenantID).Delete(&model.Session{}).Error
}
