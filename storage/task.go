...
func CreateTask(task *model.AgentTask) error {
...
func UpdateTaskStatus(id uint64, status string, errMsg string) error {
...
func GetTaskByID(tenantID, id uint64) (*model.AgentTask, error) {
	var t model.AgentTask
	err := DB.Where("id = ? AND tenant_id = ?", id, tenantID).First(&t).Error
	if err != nil {
		return nil, err
	}
	return &t, nil
...
func ListTasks(tenantID uint64, page, pageSize int) ([]model.AgentTask, int64, error) {
	...
	// 租户过滤是底线
	query := DB.Model(&model.AgentTask{}).Where("tenant_id = ?", tenantID)
