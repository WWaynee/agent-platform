...
> - **依赖故障验证的覆盖口径（如实）**：Redis 挂掉（引擎不 panic、可兜底回答）与 RabbitMQ 挂掉（不 panic + 自动重连）有**专门故障单测**（`agent/engine/fault_redis_test.go`、`mq/reconnect_test.go`）；MySQL/MinIO/Qdrant 走「访问失败即返回明确错误/降级」+ `/health` 探活（`TestHealthHandler_DownReturns503` 单测），但**未实际停止容器做端到端注入**，属仅代码/探活级覆盖——如需更硬核验证可后续补停容器端到端。
