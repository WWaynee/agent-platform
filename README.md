...
3. **租户 ID 传递安全（最高优先级）**
   - 现状：部分接口可能仍从前端传 `tenant_id`
   - 问题：多租户安全底线是 `tenant_id` 必须从 JWT 拿，不能信前端
   - 后续：全面排查所有私有接口，一律从 Context 拿 `tenant_id`，前端传了也忽略
...
5. **统一错误码**
   - 现状：失败可能直接回 HTTP 状态码，或 `code` 乱传
   - 后续：定义统一错误码常量，如 `40001`=参数错误、`40101`=未登录、`40301`=无权限、`50001`=服务器错误，所有接口统一使用

6. **请求日志中间件**
   - 现状：仅靠 Gin 默认日志
   - 后续：加自定义请求日志中间件，记录请求路径、方法、耗时、状态码、TraceID，便于排查

7. **CORS 中间件**
   - 现状：`cors.Default()` 允许任意来源
   - 后续：加受控 CORS 中间件，允许前端域名跨域，限制来源白名单

8. **参数校验不统一**
   - 现状：每个 handler 自己写参数校验
   - 后续：用 `validator` 库做结构体标签校验，统一参数错误处理
